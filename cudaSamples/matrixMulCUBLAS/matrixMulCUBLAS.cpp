...
void initializeCUDA(int argc, char **argv, int &devID,
  ...
  matrix_size.uiHA = bs;
  matrix_size.uiWA = emb_size; // K
  matrix_size.uiWB = emb_size; // N
...
int matrixMultiply(int argc, char **argv, int devID, sMatrixSize &matrix_size) {
  ...
  // CUBLAS version 2.0
  {
    ...
    checkCudaErrors(cublasSetMathMode(handle, CUBLAS_PEDANTIC_MATH));
    checkCudaErrors(cublasSetMathMode(handle, CUBLAS_DEFAULT_MATH));
    ...
    printf("%d, %.1f, %.2f", 
           matrix_size.uiHA, msecPerMatrixMul * 1000, flopsPerMatrixMul);
...
int main(int argc, char **argv) {
  ...
  for(int bs = 1; bs < max_bs; bs++) {
      initializeCUDA(argc, argv, devID, bs, emb_size, matrix_size);
